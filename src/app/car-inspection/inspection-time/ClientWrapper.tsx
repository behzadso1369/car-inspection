...
export default function ClientWrapper() {
  ...
  return (
    <div className="bg-white font-IranSans lg:relative">
      ...
      {shouldShowTimeTabs && (
        <Tabs value={defaultTab} onValueChange={handleTabChange} className="w-full mt-4 bg-white font-IranSans" dir="rtl">
          <div className="sticky top-0 z-20 border-b border-[#EAEAEA] bg-white px-1 pt-2 pb-0 shadow-[0_4px_12px_-8px_rgba(16,24,40,0.12)]">
            <TabsList className="h-auto w-full justify-between overflow-x-hidden rounded-none bg-transparent p-0">
              {carInspectionDateTime.map((item: any) => (
                <TabsTrigger
                  key={item.Id}
                  className="flex min-w-0 flex-1 flex-col whitespace-normal text-[#55565A] data-[state=active]:text-[#416CEA] data-[state=active]:!border-b data-[state=active]:border-b-[#416CEA] data-[state=active]:shadow-none px-1 mx-0.5 pb-3"
                  value={item.Id}
                >
          ...
          <TabsContent value={defaultTab} className="mt-4 grid grid-cols-2 gap-2 px-2 pb-24">
